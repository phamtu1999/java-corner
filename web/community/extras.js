...
import {progressProfile} from './catalog-tools.js';
...
export async function enhanceExtras({root,r,user,api,esc,openModal,notify,refresh}){
 ...
 if(r.query.has('profile')){
  ...
  section.insertAdjacentHTML('beforeend',progressProfile(p.progress,esc));
  root.replaceChildren(section);
 ...
 if(r.view==='admin'&&user?.role==='admin'){
  ...
  const audit=async page=>{const rows=await api('/admin/audit?page='+page);openModal(`<h2 id="modal-title">Lịch sử quản trị · Trang ${page}</h2>${rows.map(x=>`<details class="review"><summary>${esc(x.actor||'Hệ thống')} · ${esc(x.action)} · ${esc(x.after_data?.title||x.before_data?.title)} · ${new Date(x.created_at).toLocaleString('vi-VN')}</summary><strong>Trước</strong><pre>${esc(JSON.stringify(x.before_data,null,2))}</pre><strong>Sau</strong><pre>${esc(JSON.stringify(x.after_data,null,2))}</pre></details>`).join('')||'<p>Chưa có thay đổi.</p>'}`);const box=document.querySelector('#modal-body');if(page>1)box.append(button('Trang trước',()=>audit(page-1)));if(rows.length===30)box.append(button('Trang sau',()=>audit(page+1)));};
  toolbar.append(button('Lịch sử quản trị',()=>audit(1)));
  const health=async page=>{
   const data=await api('/admin/health?page='+page);
   openModal(`<h2 id="modal-title">Kiểm tra kho game · Trang ${page}</h2><p>Mỗi trang 20 phiên bản. Inspector kiểm tra từng tệp, tối đa 5 lượt/phút. Không xóa hoặc sửa game.</p><div class="health-items">${data.items.map(g=>`<article class="review health-item"><a href="/game/${esc(g.id)}"><strong>${esc(g.title)}</strong></a><small>${esc(g.filename)}</small><p>${esc(g.file)} · ${esc(g.archive||'')} · ${g.has_icon?'Có ảnh':'Thiếu ảnh'} · ${esc(g.screen||'Thiếu màn hình')}</p>${(g.duplicates||[]).map(d=>`<p>Trùng tệp: <a href="/game/${esc(d.id)}">${esc(d.title)}</a></p>`).join('')}<div class="health-action"></div><div class="health-result" aria-live="polite"></div></article>`).join('')||'<p>Không còn game.</p>'}</div>`);
   const box=document.querySelector('#modal-body');
   box.querySelectorAll('.health-item').forEach((row,index)=>{
    const g=data.items[index],result=row.querySelector('.health-result');
    row.querySelector('.health-action').append(button('Kiểm tra JAR · Inspector',async()=>{
     result.textContent='Đang tải và phân tích JAR…';result.setAttribute('aria-busy','true');
     try{const report=await api('/admin/games/'+encodeURIComponent(g.id)+'/verify-archive',{method:'POST'});result.innerHTML=`<p>${esc(report.message)}</p><details open><summary>Báo cáo Inspector</summary>${renderJarInspection(report.inspection,esc)}</details>`;}
     catch(error){result.textContent='Chưa kiểm tra được: '+error.message;}
     finally{result.removeAttribute('aria-busy');}
    }));
   });
   if(page>1)box.append(button('Trang trước',()=>health(page-1)));
   if(data.hasMore)box.append(button('Kiểm tra 20 bản tiếp',()=>health(page+1)));
  };toolbar.append(button('Kiểm tra game lỗi',()=>health(1)));
 }
}
