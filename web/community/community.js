...
async function render() {
  ...
  try {
    ...
    if (['games','history','mine'].includes(r.view)) {
      ...
      html = `<a class="breadcrumb" href="/${g.visibility === 'private' ? 'mine' : 'games'}">← Trở về thư viện</a>` + heading(g.family_title || g.title, `${g.category || 'Game cá nhân'} · ${g.variants.length > 1 ? `${g.variants.length} phiên bản` : size(g.size)} · ${g.visibility === 'public' ? 'Công khai' : 'Chỉ mình bạn'}`, `<a class="button primary" href="${g.variants?.length > 1 ? '#versions' : play(g.id)}">${g.variants?.length > 1 ? 'Chọn phiên bản' : 'Chơi ngay'}</a>`) + `<div class="detail-grid"><section class="panel"><h2>Về trò chơi</h2><div class="prose">${esc(g.description || 'Chưa có mô tả cho game này.')}</div>${canEdit ? `<div class="actions">${button('edit-game','Sửa thông tin',g.id,'small')}${button('delete-game','Xóa phiên bản',g.id,'small danger')}</div>` : ''}</section><aside class="panel"><h3>Trước khi chơi</h3><p class="muted">Lần đầu sẽ tải bộ chạy Java. Có thể đổi sang bố cục Nokia cổ điển trong màn chơi.</p><p class="muted">Lịch sử chơi lưu theo tài khoản. Tiến trình trong game lưu trên trình duyệt này.</p><a class="button small" href="/library">Quản lý bản lưu</a></aside></div>`;
      ...
      if (discussions) html += `<div class="heading"><h2>Thảo luận về game</h2>${button('new-post','Viết bài',g.id,'small')}</div>${topics(discussions.items)}${pager(discussions,r)}`;
