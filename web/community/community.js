...
async function render() {
  ...
  try {
    ...
    if (['games','history','mine'].includes(r.view)) {
      ...
      html = `<a class="breadcrumb" href="/${g.visibility === 'private' ? 'mine' : 'games'}">← Trở về thư viện</a><section class="game-detail-hero panel"><div class="game-detail-cover">${gameThumbnail(g)}</div><div class="game-detail-info"><p class="game-detail-publisher">${esc(g.publisher || 'Java Corner')}</p><h1>${esc(g.family_title || g.title)}</h1><div class="game-detail-rating">${g.rating ? `★ ${esc(g.rating)}` : '★ Chưa có đánh giá'}</div><div class="game-detail-tags"><span>${g.variants?.[0]?.screen || '240x320'}</span><span>${g.visibility === 'public' ? 'Online' : 'Riêng tư'}</span><span>Nokia / Java ME</span></div><a class="button primary game-detail-play" href="${g.variants?.length > 1 ? '#versions' : play(g.id)}">${g.variants?.length > 1 ? 'CHỌN PHIÊN BẢN' : 'CHƠI NGAY'}</a></div></section><nav class="game-detail-tabs" aria-label="Nội dung game"><a href="#media">Ảnh màn hình</a><a href="#description">Mô tả</a><a href="#versions">Phiên bản</a><a href="#guide">Hướng dẫn</a><a href="#comments">Bình luận</a><a href="#compatibility">Compatibility</a></nav><div class="detail-grid"><section class="panel" id="description"><h2>Mô tả</h2><div class="prose">${esc(g.description || 'Chưa có mô tả cho game này.')}</div>${canEdit ? `<div class="actions">${button('edit-game','Sửa thông tin',g.id,'small')}${button('delete-game','Xóa phiên bản',g.id,'small danger')}</div>` : ''}</section><aside class="panel" id="media"><h2>Ảnh màn hình</h2><p class="muted">Ảnh và video sẽ xuất hiện khi nhà phát hành bổ sung media.</p></aside></div>`;
