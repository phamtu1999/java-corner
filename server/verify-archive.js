...
export async function verifyArchive(url,expectedHash,fetcher=fetch) {
 ...
 try{const path=join(directory,'app.jar');await writeFile(path,bytes);await validateJar(path);return {size,sha256,message:'SHA-256 khớp; cấu trúc Java ME hợp lệ.'};}
