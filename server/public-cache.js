...
export function publicCache({ttl=10000,maxEntries=100}={}) {
 ...
 return async(req,res,next)=>{
  ...
  if(existing&&(existing.pending||existing.until>Date.now())) {
   ...
   if(data!==null&&entries.get(key)===existing)return res.json(data);
   return next();
  }
  ...
  let data=null;
  const json=res.json;
  res.json=function(body){if(res.statusCode===200)data=body;return json.call(this,body);};
  let done=false;
  const finish=()=>{
   if(done)return;done=true;entry.pending=false;entry.until=Date.now()+ttl;
