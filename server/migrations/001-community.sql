...
CREATE TABLE IF NOT EXISTS player_profiles (
 user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
 game_id TEXT NOT NULL REFERENCES games(id) ON DELETE CASCADE,
 profile JSONB NOT NULL DEFAULT '{}', revision TEXT NOT NULL,
 PRIMARY KEY(user_id,game_id)
);
ALTER TABLE player_profiles ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON player_profiles FROM PUBLIC;

ALTER TABLE game_reports ADD COLUMN IF NOT EXISTS replay JSONB;
ALTER TABLE games ADD COLUMN IF NOT EXISTS source_metadata JSONB NOT NULL DEFAULT '{}';
ALTER TABLE games ADD COLUMN IF NOT EXISTS metadata_verified JSONB NOT NULL DEFAULT '{}';
CREATE TABLE IF NOT EXISTS emulator_crashes (
 key TEXT PRIMARY KEY,game_id TEXT NOT NULL REFERENCES games(id) ON DELETE CASCADE,
 sha256 TEXT NOT NULL,runtime TEXT NOT NULL,exception TEXT NOT NULL,signature TEXT NOT NULL,
 screen TEXT NOT NULL,network BOOLEAN NOT NULL,occurrences BIGINT NOT NULL DEFAULT 1,
 last_seen TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS crashes_last_seen ON emulator_crashes(last_seen);
CREATE TABLE IF NOT EXISTS operation_counts(minute TIMESTAMPTZ NOT NULL,kind TEXT NOT NULL,hits BIGINT NOT NULL,PRIMARY KEY(minute,kind));
CREATE TABLE IF NOT EXISTS relay_instances(id TEXT PRIMARY KEY,active INTEGER NOT NULL,expires_at TIMESTAMPTZ NOT NULL);
ALTER TABLE emulator_crashes ENABLE ROW LEVEL SECURITY;
ALTER TABLE operation_counts ENABLE ROW LEVEL SECURITY;
ALTER TABLE relay_instances ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON emulator_crashes,operation_counts,relay_instances FROM PUBLIC;
