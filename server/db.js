...
export function databaseConfig(connectionString = process.env.DATABASE_URL, env = process.env) {
  ...
  // Supabase session mode reserves a backend for every serverless connection.
  if (env.VERCEL === '1' && url.hostname.endsWith('.pooler.supabase.com') && (!url.port || url.port === '5432')) url.port = '6543';
  return {
    connectionString: url.toString(),
    ssl: { ca: readFileSync(new URL('./certs/supabase-ca.crt', import.meta.url), 'utf8'), rejectUnauthorized: true },
    max: 5, connectionTimeoutMillis: 15000, idleTimeoutMillis: env.VERCEL === '1' ? 5000 : 30000,
...
export async function openDatabase({ connectionString, schema = 'public' } = {}) {
  ...
  const query = async (text, values = []) => {
    ...
    try {
      ...
      try { await client.query('ROLLBACK'); } catch { discarded = true; client.release(true); throw error; }
      throw error;
