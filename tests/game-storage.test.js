...
test('private archive links use signed storage URLs and public links need no request',async()=>{
 const calls=[];
 const store=gameStorage({env,request:async(url,options)=>{calls.push({url,options});return Response.json(options.method==='POST'?{signedURL:'/object/sign/private/a.jar?token=example'}:{public:false});}});
 ...
 assert.equal(JSON.parse(calls[1].options.body).expiresIn,60);
...
test('private writes refuse public buckets and signing refuses foreign origins',async()=>{
 ...
 const foreign=gameStorage({env,request:async(url,options)=>Response.json(options.method==='POST'?{signedURL:'https://untrusted.example/file'}:{public:false})});
